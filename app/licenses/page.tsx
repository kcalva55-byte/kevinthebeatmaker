...
async function getLicenses(): Promise<
  PublicLicense[]
> {
  ...
  if (error) {
    console.error(
      "Error loading license templates:",
      error.message,
    );
  ...
  const templates =
    (data ?? []) as LicenseTemplateRow[];

  return templates.map(
    ({ default_price, ...license }) => ({
      ...license,
      price: default_price,
    }),
  );
...
export default async function LicensesPage() {
  ...
  const premiumIndex = licenses.findIndex(
    (license) =>
      license.name
        .toLowerCase()
        .includes("premium"),
      ...
      <div className="relative mx-auto max-w-7xl px-5 py-8 sm:px-8 lg:px-10">
        ...
        <section className="pb-14 pt-16 text-center sm:pb-20 sm:pt-24">
          ...
          <h1 className="mx-auto mt-7 max-w-4xl text-4xl font-bold tracking-[-0.04em] sm:text-5xl lg:text-6xl">
            Elige la licencia correcta para tu
            proyecto
          </h1>

          <p className="mx-auto mt-6 max-w-2xl text-base leading-8 text-white/55 sm:text-lg">
            Compara los permisos, formatos y
            límites de cada licencia. Todas las
            compras incluyen entrega digital y un
        ...
        {licenses.length > 0 ? (
          ...
          <section className="rounded-3xl border border-white/10 bg-white/[0.03] px-6 py-16 text-center">
            ...
            <p className="mx-auto mt-3 max-w-md text-sm leading-6 text-white/50">
              En este momento no hay licencias
              publicadas. Intenta nuevamente más
              tarde.
            </p>
          </section>
        )}
          ...
          <div className="grid gap-5 md:grid-cols-3">
            <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-6">
              ...
              <p className="mt-2 text-sm leading-6 text-white/50">
                Obtendrás los archivos de audio
            ...
            <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-6">
              ...
              <p className="mt-2 text-sm leading-6 text-white/50">
                Recibirás un documento que confirma
                tus permisos de uso y los detalles
                de la compra.
              </p>
            </div>
            ...
            <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-6">
              ...
              <p className="mt-2 text-sm leading-6 text-white/50">
                El enlace de descarga y la
                información de la licencia se
                enviarán al correo del comprador.
              </p>
            </div>
          </div>
        </section>
